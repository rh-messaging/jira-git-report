...
public class ProjectParser {
   ...
   public static void main(String arg[]) {
      try {
         if (arg.length < REQUIRED_PARAMETER_COUNT) {
            printSyntax();
            System.exit(-1);
         }
         String otherBranches[] = new String[0];
         if (arg.length > REQUIRED_PARAMETER_COUNT) {
            otherBranches = new String[arg.length - 7];
            int c = 0;
            for (int i = REQUIRED_PARAMETER_COUNT; i < arg.length; i++) {
         ...
         switch (arg[0]) {
            case "artemis":
               artemisProcess(arg[1], arg[2], arg[3], arg[4], Boolean.parseBoolean(arg[5]), Boolean.parseBoolean(arg[6]), otherBranches);
               break;
            case "amq":
   ...
   private static void printSyntax() {
      System.err.println("use Parser <project> <repository> <reportOutput> <from> <to> <rest : true|false> <forActiveMQWeb : true|false> cherry-pick-branch1 cherry-pick-source1 cherry-pick-branch2 cherry-pick-source2...cherry-pickbranchN cherry-pick-sourceN");
      System.err.println("    valid projects: artemis, amq, amqcherry, wildfly, qpid-jms, qpid-dispatch");
   }

   private static void wildflyProcess(String clone,
                                      String output,
                                      String tag1,
                                      String tag2,
                                      boolean rest,
                                      String[] otherBranches) throws Exception {

      JiraParser jiraParser = new JiraParser("WildFly JIRAs");
      jiraParser.setJira("WFLY-").setJiraBrowseURI("https://issues.redhat.com/browse/").setSampleJQL("https://issues.redhat.com/issues/?jql=project%20%3D%20WildFly%20AND%20KEY%20IN");

      if (rest) {
         jiraParser.setRestLocation("https://issues.redhat.com/rest/api/2/issue/");
      }

      GitParser parser = new GitParser(new File(clone), "https://github.com/wildfly/wildfly/").setSourceSuffix(".java", ".md", ".c", ".sh", ".groovy", ".adoc");
   ...
   private static void artemisProcess(String clone,
                                      String output,
                                      String tag1,
                                      String tag2,
                                      boolean rest,
                                      boolean forActiveMQWeb,
                                      String[] otherBranches) throws Exception {

      JiraParser jiraParser = new JiraParser("ARTEMIS JIRAs");

      jiraParser.setJira("ARTEMIS-").setJiraBrowseURI("https://issues.apache.org/jira/browse/").setSampleJQL("https://issues.apache.org/jira/issues/?jql=project%20%3D%20ARTEMIS%20AND%20key%20in%20");

      if (rest) {
         jiraParser.setRestLocation("https://issues.apache.org/jira/rest/api/2/issue/");
      }

      GitParser parser = new GitParser(new File(clone), "https://github.com/apache/activemq-artemis/").setSourceSuffix(".java", ".md", ".c", ".sh", ".groovy");
      parser.addJIRA(jiraParser);
      ...
      File file = new File(output);
      parser.parse(file, tag1, tag2, forActiveMQWeb);
   }

   private static void qpidJMSProcess(String clone,
                                      String output,
                                      String tag1,
                                      String tag2,
                                      boolean rest,
      ...
      jiraParser.setJira("QPIDJMS-").setJiraBrowseURI("https://issues.apache.org/jira/browse/").setSampleJQL("https://issues.apache.org/jira/issues/?jql=project%20%3D%20QPIDJMS%20AND%20key%20in%20");

      if (rest) {
         jiraParser.setRestLocation("https://issues.apache.org/jira/rest/api/2/issue/");
      }

      GitParser parser = new GitParser(new File(clone), "https://github.com/apache/qpid-jms/").setSourceSuffix(".java", ".md", ".c", ".sh", ".groovy");
      parser.addJIRA(jiraParser);
   ...
   private static void qpidDispatchProcess(String clone,
                                           String output,
                                           String tag1,
                                           String tag2,
                                           boolean rest,
      ...
      jiraParser.setJira("DISPATCH-").setJiraBrowseURI("https://issues.apache.org/browse/").setSampleJQL("https://issues.apache.org/jira/issues/?jql=project%20%3D%20DISPATCH%20AND%20key%20in%20");

      if (rest) {
         jiraParser.setRestLocation("https://issues.apache.org/jira/rest/api/2/issue/");
      }

      GitParser parser = new GitParser(new File(clone), "https://github.com/apache/qpid-dispatch/").setSourceSuffix(".java", ".md", ".c", ".sh", ".groovy", ".py", ".h");
      parser.addJIRA(jiraParser);
   ...
   private static void amqProcess(String clone,
                                  String output,
                                  String tag1,
                                  String tag2,
                                  boolean rest,
                                  String[] otherBranches,
                                  boolean treatCherryPick) throws Exception {

      GitParser parser = new GitParser(new File(clone), "https://github.com/rh-messaging/activemq-artemis/").setSourceSuffix(".java", ".md", ".c", ".sh", ".groovy");

      JiraParser artemisJIRA = new JiraParser("ARTEMIS");
      artemisJIRA.setJira("ARTEMIS-").setJiraBrowseURI("https://issues.apache.org/jira/browse/").setSampleJQL("https://issues.apache.org/jira/issues/?jql=project%20%3D%20ARTEMIS%20AND%20key%20in%20");
      ...
      entmqbrJIRA.setJira("ENTMQBR-").setJiraBrowseURI("https://issues.redhat.com/browse/").setSampleJQL("https://issues.redhat.com/issues/?jql=project%20%3D%20ENTMQBR%20AND%20KEY%20IN");
